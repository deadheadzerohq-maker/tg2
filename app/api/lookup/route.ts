...
// TODO: Replace this with real FMCSA / data provider call
async function fetchCarrierData(value: string): Promise<CarrierRaw | null> {
  const normalized = value.replace(/[^0-9]/g, "");
  if (!normalized) return null;

  // Mock response for now so the app works end-to-end
  return {
...
export async function POST(req: Request) {
  try {
    ...
    // TODO: Check user auth + subscription status using Supabase
